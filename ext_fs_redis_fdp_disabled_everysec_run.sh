...
RESULTS_DIR="/home/solesie/redis/bench-results/ext/${TYPE}/last5"
...
sudo ${REDIS_BENCH} \
    -h 127.0.0.1 -p 6379 \
    -c 50 -n 7046589 -d 4096 -t set -r 5321523 -k 1 
...
for PHASE in $(seq "${START_PHASE}" "${END_PHASE}"); do
    ...
    echo "    → redis-benchmark (90GiB, RPS 기록 → ${RPS_FILE})"
    sudo ${REDIS_BENCH} \
        -h 127.0.0.1 -p 6379 \
        -c 50 \
        -n 23592960 -d 4096 -t set -r 5321523 -k 1 \
